...
class Jualproduk extends Controller
{
    ...
    public function datatable(){    
        return datatables()->of(Produk::query()->where('user_id',Auth::user()->id))
        ...
        ->addColumn('operation', function($data){
            $operation = '<div class="">
            <a href="'.route('editproduk', $data->id) .'"><button type="button" class="btn btn-success">Edit</button></a>
            <a href="'.route('product.destroy', $data->id) .'"><button type="button" class="btn btn-danger">Delete</button></a>
            </div>';
            return $operation;
        })
